...
var name = "To Infinity! on Convergents to √2";
...
var getPrimaryEquation = () => {
    ...
    if (q1Exp.level == 1) result += "^{1e9}";
    if (q1Exp.level == 2) result += "^{1e30}";
    if (q1Exp.level == 3) result += "^{1e100}";
    result += "q_2q\\\\\\dot{q}=c_1"
    if (c2.isAvailable)
    {
        result += "c_2";
        if (c2Exp.level == 1) result += "^{1e10}";
        if (c2Exp.level == 2) result += "^{1e100}";
    }
...
var tt1250 = BigNumber.TEN.pow(1e125);
var multcutoff = BigNumber.from(1.18568685283083)*BigNumber.TEN.pow(1e100)
...
var getTau = () => currency.value.pow(1e200);
